...
HANDLE *dev_handle;
long long int partition_size,blocks,groups,gdt_blocks,free_blocks;
...
int _getch(void);
int rand_s(unsigned int *buf);
int set_pos(unsigned long long val)
{
	LARGE_INTEGER buf;
	buf.QuadPart=val;
	SetFilePointerEx(dev_handle,buf,NULL,FILE_BEGIN);
}
...
long long dev_size(void)
{
	...
	if(GetFileSizeEx(dev_handle,(void *)buf))
	{
		return buf[0];
	}
	return -1;
}
void fill_rand(void *buf)
{
	int n;
	n=16;
	do
...
void buf_write(void *ptr,unsigned long long int size)
{
	unsigned int n;
	while(size)
	{
...
void buf_write_zero(unsigned long long int size)
{
	unsigned int n;
	while(size)
	{
...
void write_group(unsigned long long n,int has_super)
{
	set_pos(n<<27);
	if(has_super)
	{
...
void print_progress(long long groups,long long progress)
{
	printf("\rFormatting Progress: %lld%%",progress*100/groups,1);
}
...
int main(void)
{
	...
	char buf[256];
	printf("Input the drive (like \"\\\\.\\D:\") you want to format: ");
	get_s(buf,255);
	dev_handle=CreateFileA(buf,GENERIC_READ|GENERIC_WRITE,0,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
